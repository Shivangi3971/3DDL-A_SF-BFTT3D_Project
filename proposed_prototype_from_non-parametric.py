...
def main():
    ...
    for i in test_corruptions:
        ...
        # # Prepare target data only
        # test_loader = DataLoader(ModelNet40C(split='test', corruption='none'), 
        ...
        for points, labels in tqdm(test_loader):
            ...
            # logits = source_model(points).detach()
            logits = point_nn(points).detach()
            entropy = softmax_entropy(logits)

            pseudo_labels = nn.Softmax(dim=1)(logits).argmax(dim=1) #logits_source.argmax(dim=1)  # The predicted class is the one with the max logit
            labels =pseudo_labels

            low_entropy_idx = entropy < entropy.mean()
            high_entropy_idx = ~low_entropy_idx

            low_entropy_features.append(logits[low_entropy_idx])
            high_entropy_features.append(logits[high_entropy_idx])
            label_memory.append(labels[low_entropy_idx].cuda())
            high_label_memory.append(labels[high_entropy_idx].cuda())

            feature_memory.append(logits)
        ...
        # Create Prototypes from Low-Entropy Features
        ...
        prototypes, label_memory = prototype_cal(low_entropy_features.cuda(), label_memory_ys.cuda(), label_memory.shape[1])
    

        high_label_memory_ys = torch.cat(high_label_memory, dim=0)
        high_label_memory = F.one_hot(high_label_memory_ys, num_classes=label_memory.shape[1]).float()
        ...
        for counter in range(n_batches):
            ...
            # Adjust the shape of logits to match logits_source_curr if the final batch size is smaller than bz
            if logits.shape[0] != logits_source_curr.shape[0]:
                min_batch_size = min(logits.shape[0], logits_source_curr.shape[0])
                logits = logits[:min_batch_size]
                logits_source_curr = logits_source_curr[:min_batch_size]
