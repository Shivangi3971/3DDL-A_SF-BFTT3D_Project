...
def main():
    ...
    for i in test_corruptions:
        ...
        # high_freq_features = extract_high_frequency_features(feature_memory, low_cut=0.5)
        high_freq_features = extract_high_frequency_features(feature_memory, low_cut=0.9)

        # Cluster features into 40 classes
        feature_memory, cluster_labels = cluster_high_frequency_features(high_freq_features, num_classes=40)
...
def extract_high_frequency_features(features, low_cut=0.7):
    """
    Extract high-frequency components from features using FFT.

    Args:
        features (torch.Tensor): Input features of shape (N, D), where N is the number of samples and D is the feature dimension.
        low_cut (float): Fraction of frequencies to retain as high-frequency components.
    
    Returns:
        torch.Tensor: High-frequency features of shape (N, D).
    """
    # Apply FFT
    ...
    N, D = features.shape
    cutoff = int(D * low_cut)
    high_freq_features = torch.zeros_like(fft_features)
    high_freq_features[:, cutoff:] = fft_features[:, cutoff:]

    # Convert back to the spatial domain
    high_freq_features = torch.fft.ifft(high_freq_features, dim=-1).real
    return high_freq_features


from sklearn.cluster import KMeans

def cluster_high_frequency_features(features, num_classes=40):
    """
    Cluster high-frequency features into `num_classes` clusters.

    Args:
        features (torch.Tensor): High-frequency features of shape (N, D).
        num_classes (int): Number of clusters (classes).
    
    Returns:
        torch.Tensor: Cluster centers (prototypes) of shape (num_classes, D).
    ...
    # Perform clustering
    kmeans = KMeans(n_clusters=num_classes, random_state=0).fit(features_np)
    cluster_centers = torch.tensor(kmeans.cluster_centers_, dtype=torch.float).cuda()
    cluster_labels = torch.tensor(kmeans.labels_, dtype=torch.long).cuda()
