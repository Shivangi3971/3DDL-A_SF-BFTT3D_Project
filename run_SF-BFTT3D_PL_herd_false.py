...
def main():
    ...
    test_c = ["lidar"]
    # test_corruptions = ['gaussian','uniform','background','impulse', 'upsampling', 'distortion_rbf',
    #                     'distortion_rbf_inv', 'density', 'density_inc', 'shear', 'rotation', 'cutout', 'distortion', 'occlusion', 'lidar']

    # test_corruptions = ['background']
    
    test_corruptions = ['gaussian','uniform','impulse', 'upsampling', 'distortion_rbf',
                        'distortion_rbf_inv', 'density', 'density_inc', 'cutout', 'distortion', 'lidar','shear', 'occlusion', 'rotation','background']
    ...
    for i in test_corruptions:
        ...
        for points_cpu, labels in tqdm(test_loader):
            ...
            # Pseudo-label generation: select the predicted class with highest confidence
        ...
        label_memory_ys = label_memory_ys.to(torch.int64)
        label_memory = F.one_hot(label_memory_ys).float()#.squeeze().float()
        ...
        # Prototype
        feature_memory, label_memory = protoype_cal(feature_memory, label_memory_ys, label_memory.shape[1], herding = False) #True)
        ...
        # if i == 'uniform':
        #      print("Saving uniform prototypes.")
        #      uniform_prototypes = feature_memory.clone()
        #      uniform_labels = label_memory.clone()
        #      #continue
        ...
        if gaussian_prototypes is not None:
            for class_id in range(40):
                if not (label_memory_ys == class_id).any():
                    print(f"Class {class_id} missing in {i}. Using uniform prototype.")
        ...
        for counter in range(n_batches):
            ...
            # logits = generate_logits_from_similarity(Sim, gamma=0.5)#1.0)
