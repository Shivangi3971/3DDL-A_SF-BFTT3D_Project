...
class TCA:
    def __init__(self, kernel_type='primal', dim=200, lamb=1, gamma=1):
        '''
        Init func
    ...
    def fit(self, Xs, Xt):
        ...
        # # Ensure both Xs and Xt are 2D
        # if Xs.ndim == 1:
        #     Xs = np.expand_dims(Xs, axis=1)
        # if Xt.ndim == 1:
        #     Xt = np.expand_dims(Xt, axis=1)
        
        #  # Ensure Xs and Xt have compatible shapes
        # if Xs.shape[1] != Xt.shape[1]:
        #     target_dim = min(Xs.shape[1], Xt.shape[1])  # Match to smaller dimension
        #     pca = PCA(n_components=target_dim)
        #     if Xs.shape[1] > target_dim:
        #         Xs = pca.fit_transform(Xs)
        #     if Xt.shape[1] > target_dim:
        #         Xt = pca.fit_transform(Xt)

        # # Ensure the same number of samples for stacking
        # min_samples = min(Xs.shape[0], Xt.shape[0])
        # Xs = Xs[:min_samples]
        # Xt = Xt[:min_samples]
