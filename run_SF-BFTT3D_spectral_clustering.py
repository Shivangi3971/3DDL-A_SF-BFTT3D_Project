...
def main():
    ...
    for i in test_corruptions:
        ...
        # Perform clustering to generate prototypes
        num_clusters = 40  # Number of classes in ModelNet40
        # Perform Spectral Clustering to generate prototypes
        feature_memory, cluster_labels = spectral_clustering_prototype(feature_memory, num_clusters, device='cuda')
        ...
        for counter in range(n_batches):
            ...
            logits = generate_logits_from_similarity(Sim, gamma=1.0)
...
def spectral_clustering_prototype(test_features, num_clusters, device='cuda'):
    """
    Perform Spectral Clustering on the test features to generate prototypes.
    
    Args:
        test_features (torch.Tensor): The features extracted from the test set (num_samples, feature_dim).
        num_clusters (int): The number of clusters (prototypes).
        device (str): The device to use ('cuda' or 'cpu').
    
    Returns:
        torch.Tensor: The prototypes based on the cluster centroids.
        torch.Tensor: The cluster assignments for each sample.
    """
    # Convert test_features to numpy array for spectral clustering
    features_cpu = test_features.cpu().numpy()

    # Spectral Clustering
    clustering = SpectralClustering(n_clusters=num_clusters, affinity='nearest_neighbors', random_state=42)
    cluster_labels = clustering.fit_predict(features_cpu)  # Cluster assignment for each sample
    
    # Get the centroids (prototypes) by taking the mean feature vector of each cluster
    prototypes = []
    for i in range(num_clusters):
        cluster_features = features_cpu[cluster_labels == i]
        cluster_center = cluster_features.mean(axis=0)
        prototypes.append(cluster_center)
    
    prototypes = torch.tensor(np.array(prototypes), dtype=torch.float).to(device)
    cluster_labels = torch.tensor(cluster_labels, dtype=torch.long).to(device)
    
    return prototypes, cluster_labels
...
def mean_square_distance_batch(batch_vector1, vector2):
    ...
    print("batch_vector1 shape:", batch_vector1.shape)
